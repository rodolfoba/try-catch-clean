...
public class OptionalMain {
    ...
    static class AuthenticatorWrapper {
        ...
        public void login() throws Exception {
            user = tryLogin()
                    .orElseGet(() -> tryGmailLogin()
                            .orElseThrow(() -> new AuthenticatorException()));
            authenticator.twoFactor(user, twoFactorPwd);
        }
        
        private Optional<User> tryLogin() {
            System.out.println("try login...");
            Optional<User> optional;
            try {
                optional = Optional.of(authenticator.login(userId, pwd));
            } catch (Exception e) {
        ...
        private Optional<User> tryGmailLogin() {
            System.out.println("try GmailLogin");
            Optional<User> optional;
            try {
                optional = Optional.of(authenticator.gmailLogin(userId, pwd));
            } catch (Exception e) {
                optional = Optional.empty();
            }
